...
```
-> 버튼 클릭했을 때 브라우저가 action="/login" 주소롤 데이터를 보내버린다.
-> 3000 -> 8000
-> 페이지가 새로고침 되거나 이동됨
...
- React/View에서 form을 쓰면 이벤트가 적용되지 않고 새로고침이 됨
- 화면이 SPA인데 갑자기 SSR처럼 이동되어 버림

사례
사례1 - 버튼을 눌렀는데 페이지가 새로고침 됨

사례2 - JS에서 이벤트 발동하기 전에 submit되어 버렸다???

사례3 - AJAX로 처리하는데 새로고침이 발동함

사례4 - 리액트에서도 form이 들어가면 새로고침이 됨 - form태그 쓰지마

해결방법
첫번째
- form의 기본 submit동작을 막아버려 -> e.preventDefault()
두번째
